...
suite("AuthenticatedClient", () => {
  const api_url = new URL(index_js_1.ApiUri);
  const key = "poloniex-api-key";
  const secret = "poloniex-api-secret";
  const client = new index_js_1.AuthenticatedClient({ key, secret });
  const nonce = 1;
  client.nonce = () => nonce;
  const body = (form) => (string) =>
  ...
  test("constructor (default `currencyPair`)", () => {
    (0, node_assert_1.deepStrictEqual)(
      client.currencyPair,
      index_js_1.DefaultPair
    );
  });
  test("constructor", () => {
    const currencyPair = "BTC_ETH";
    const otherClient = new index_js_1.AuthenticatedClient({
      currencyPair,
      key,
      secret,
    });
    (0, node_assert_1.deepStrictEqual)(otherClient.currencyPair, currencyPair);
  });
  test(".post()", async () => {
    const response = { success: 1 };
    const path = "/";
    const url = new URL(path, index_js_1.ApiUri);
    mockPool
  ...
  test(".post() (with error in the response)", async () => {
    ...
    try {
      ...
      (0, node_assert_1.fail)("Should throw an error");
    } catch (err) {
      (0, node_assert_1.deepStrictEqual)(err, new Error(error));
    }
  });
  test(".post() (with no success in the response)", async () => {
    ...
    try {
      ...
      (0, node_assert_1.deepStrictEqual)(err, new Error(result.error));
    }
  });
  ...
  test(".getDepositAddresses()", async () => {
    ...
    (0, node_assert_1.deepStrictEqual)(data, addresses);
  });
  ...
  test(".getOpenOrders() (with no `currencyPair`)", async () => {
    ...
    const currencyPair = index_js_1.DefaultPair;
    const path = "/tradingApi";
    const url = new URL(path, index_js_1.ApiUri);
    const form = { command: "returnOpenOrders", currencyPair };
    mockPool
  ...
  test(".withdraw()", async () => {
    const response = { response: "Withdrew 0.000152 ETH." };
    const currency = "ETH";
  ...
  test(".nonce()", () => {
    const otherClient = new index_js_1.AuthenticatedClient({ key, secret });
    const otherNonce = otherClient.nonce();
    (0, node_assert_1.deepStrictEqual)(typeof otherNonce, "number");
  });
});
